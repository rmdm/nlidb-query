...
NlidbQuery.stream.Stringify = StringifyStream;
...
NlidbQuery.prototype.streamRelation = function  (rel, isLast, streamFrom) {
  ...
  var projectStream = new ProjectStream(isLast? null : rutil.project(rel));    
  ...
  if (streamFrom) {
    ...
    this.db.streamData(rel.rel, rutil.match(rel)).pipe(functionalPlumbing.head);
  }
...
NlidbQuery.prototype.nextFunctionalStream = function (rel) {
  var nextF = rutil.nextFIdx(rel, this.functions);
  if (nextF === -1) {
    return null;
  }
  ...
  var f = this.functions[firstF](rel, kvf.k, kvf.v, kvf.f);    
