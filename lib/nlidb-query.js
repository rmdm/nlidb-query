  ...
  return this.collectManyLevelResults(query, function onNextLvl(query, res, buf, i, j) {   
    ...
    if(k){
      for(; k--;){
        var p = res[k].path;
        p.unshift(j);
        buf.push({path: p, stream: that.streamRelation(rutil.copy(query[i][j]), !i, res[k].stream)});
      }
    } else {          
      buf.push({path: [j], stream: that.streamRelation(rutil.copy(query[i][j]), !i)});
    }
  });
}
...
NlidbQuery.prototype.streamRelation = function  (rel, isLast, streamFrom) {
  ...
  if (streamFrom) {
    ...
    if (!isLast && Object.keys(m).length === 0 && functionalPlumbing.bare) {
