  ...
  return this.collectManyLevelResults(query, function onNextLvl(query, res, buf, i, j) {   
    ...
    if(k){
      for(; k--;){
        ...
        buf.push({path: p, stream: stream, lastQuery: stream.lastQuery, lastRel: stream.rel});
      }
    } else {
      var stream = that.streamRelation(rutil.copy(query[i][j]), !i);
      buf.push({path: [j], stream: stream, lastQuery: stream.lastQuery, lastRel: stream.rel});
    }
  });
}
