module.exports = function (db, links, functions) {
  ...
  function process (query) {
    return buildStreamChains(query, {db: db, functions: functions, links: links});
  }
  
  function buildStreamChains (query, opt) {
  ...
  function streamRelation (rel, opt) {
    ...
    if (opt.streamFrom) {
      ...
      matchStream.on('data', function (data) {
        opt.db.streamData(data, rel.rel).pipe(functionalPlumbing.head);
      });
    } else {
